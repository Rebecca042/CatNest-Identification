...
def clustering_and_tree(ellipse_mask, mask_type, selected_k, max_iter, image_rgb):
    ...
    start_x, end_x, start_y, end_y  = sc.select_area(image_marked_shape, 20, 550, 20, 270)
    wall_sample = (start_y, end_y,start_x, end_x)
    mask_wall = sc.select_rectangle(wall_sample, image_rgb_shape)
    ...
    X2 = features_all[cluster_mask_flat2]
    ...
    y2 = y_full[cluster_mask_flat2]

    # Visualize mask area for features
    plt.imshow(cluster_mask_flat2.reshape(image_rgb.shape[:2]))
    plt.title('Cluster mask')
    plt.show()

    # Visualize label area
    plt.imshow(y_full.reshape(image_rgb.shape[:2]))
    ...
    tree_predictions_masked = tree.predict(X2)
    ...
    plt.barh(y=["softness", "brightness"], width=importances)
    plt.title("Feature Importances from Decision Tree")
    plt.show()

    # Visualize the decision tree predictions
    plt.imshow(tree_predictions, cmap='tab10')
    plt.title("Decision Tree Predictions on the Cat Nest Cluster")
    plt.axis("off")
    plt.show()

    unique_labels = np.unique(y2)
    class_names = ["Other", "Cat-Bed", "Wall"]
    # Adjust the indexing by subtracting 1 from unique_labels if class labels start from 1
    selected_class_names = [class_names[i] for i in unique_labels]
    print("The labels are: " + str(unique_labels))

    # Visualize the tree
    plt.figure(figsize=(8, 6))
    plot_tree(tree, feature_names=["softness", "brightness"], class_names=selected_class_names, filled=True)
    plt.title("Refinement Tree for the Image")
    plt.show()
    ...
    for node in np.unique(tree_predictions):
        mask_tmp = (tree_predictions == node) & (labels_masked3 ==  best_cluster_label2)
    ...
    for node_id in range(node_classes.shape[0]):
        ...
        print("All class index are: " + str(node_classes[node_id]))
